      ...
      <Card>
        <div className="grid md:grid-cols-[1fr_auto] gap-4 items-end">
          <div className="grid sm:grid-cols-3 gap-3">
            ...
            <label className={`grid gap-1 ${origin==='custom' ? 'opacity-100' : 'opacity-50'}`}>
              <span className="text-sm text-slate-600">Eigener Ort</span>
              <input
                className="border rounded-xl px-3 py-2 bg-white/90"
          ...
          {/* Route öffnen */}
          <a
            className={`inline-flex justify-center items-center px-5 py-3 rounded-xl text-white shadow-soft transition
                        ${routeDisabled ? 'bg-slate-300 cursor-not-allowed' : 'bg-gradient-to-tr from-emerald-500 to-orange-400 hover:scale-[1.02]'}`}
            href={mapsUrl}
            target={routeDisabled ? '_self' : '_blank'}
            rel="noreferrer"
            onClick={e => { if (routeDisabled) e.preventDefault() }}
            title={routeDisabled ? 'Bitte zuerst einen Ort eingeben' : 'In Google Maps öffnen'}
          >
      ...
      <div className="grid md:grid-cols-2 gap-4 mt-4">
        <Card title="Adresse">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <a className="px-3 py-2 rounded-xl bg-white shadow-soft"
               target="_blank"
          ...
          {/* Autobahnen */}
          <div className="mt-4">
            <div className="text-sm text-slate-600 mb-1">Gute Anbindung über:</div>
            <div className="flex flex-wrap gap-2">
              <span className="px-3 py-1 rounded-full bg-white shadow-soft border text-sm">A43 · AS Witten-Heven/Herbede</span>
              <span className="px-3 py-1 rounded-full bg-white shadow-soft border text-sm">A44 · AS Witten-Annen / Bochum-Querenburg</span>
              <span className="px-3 py-1 rounded-full bg-white shadow-soft border text-sm">A45 (i. V. mit A40/A44)</span>
            </div>
            <p className="text-slate-600 text-sm mt-2">
              Folge den Schildern Richtung <b>Kemnader See / Heveney</b>. Die Zufahrt „Luhnsmühle“ führt direkt zur Location.
            </p>
          </div>
        </Card>
      ...
      <div className="grid lg:grid-cols-3 gap-4 mt-4">
        <Card title="Parken & Ankunft">
          <ul className="list-disc pl-6">
            ...
            <li>Drop-off nah am Eingang möglich (bitte Ein- und Ausfahrt freihalten).</li>
        ...
        <Card title="ÖPNV – so kommst du hin">
          <ul className="list-disc pl-6">
            <li>Nahe Ziel-Haltestelle: <b>Freizeitbad Heveney</b> (Fußweg wenige Minuten).</li>
            <li>Ab <b>Witten Hbf</b> bzw. regional mit Bus/Bahn; je nach Startort variieren Verbindungen.</li>
            <li>Plane tagesaktuell über VRR/DB oder Google Maps (ÖPNV-Modus).</li>
          </ul>
          <div className="mt-3 flex flex-wrap gap-2">
            {['duesseldorf','reken','dortmund','muenster','duelmen','luedinghausen'].map(key => (
              <a
                key={key}
                className="px-3 py-2 rounded-xl bg-white shadow-soft"
                target="_blank"
                rel="noreferrer"
                href={routeUrl(ORIGINS[key].query || 'Witten', MODES.transit.param)}
              >
          ...
          <p className="text-slate-600 text-sm mt-2">
            Hinweis: exakte Linien/Fahrzeiten tagesaktuell prüfen (VRR/DB/Google Maps).
          </p>
        </Card>

        <Card title="Noch hilfreich">
          <ul className="list-disc pl-6">
            <li><b>Adresse fürs Navi:</b> Luhnsmühle 2, 58455 Witten</li>
            <li><b>Taxi/Rideshare:</b> Abholung am Parkplatz „BlueBeach“ (gut beschildert).</li>
            <li><b>Weg im Gelände:</b> Den Sandwegen und Palmen folgen 🌴</li>
          </ul>
        </Card>
      </div>
    </div>
